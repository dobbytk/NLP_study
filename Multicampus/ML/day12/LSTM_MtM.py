...
px = data[-t_step:].reshape(1, t_step, 1) # None 데이터 들어오는대로, 미정 
y_pred = model_1.predict(px)

# LSTM 모델 이해하기
y_pred[0][0][0] # 첫 번째 행의 50개, t=1일 때의 처음 hidden layer의 아웃풋 값

y_pred[0][0][-1] # 맨 마지막 lstm cell의 h를 뽑아라

y_pred[1][0] # last_h 위의 코드랑 출력이 같다.

y_pred[2][0] # last_c 다음 스텝으로 넘겨줄 정보
