...
def build_train_data(data, t_step, n_jump = 1):
    ...
    y = [data[(i+1):(i+t_step+1), :] for i in m]      # target data

    # shape을 조정한다. feature = 3D, target = 3D
...
# return_sequences = True로 하면 중간 출력을 모두 사용하겠단 뜻.
x_lstm = LSTM(n_hidden, return_sequences=True)(x_input) 
# 중간 출력마다 FFN을 달아서 중간 결과가 나오도록 학습시킨다.
# output에는 TimeDistributed를 사용한다.
...
for i in range(n_future):
    ...
    # y_hat t_step만큼 나올 거다. (1, 20, 1) [-1] 맨 마지막꺼
