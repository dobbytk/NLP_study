...
class Decoder:
    ...
    def __call__(self, x, enc_output, lookahead_mask, padding_mask):
      """
      모델 생성
      trainXE = Input(batch_shape=(None, 15))
      e_output, _ = Encoder(6, 512, 8, 512, 9000)(trainXE, mask)

      x: decoder의 입력
      enc_output: 인코더의 출력 
      trainXD = Input()
      d_output, _, _ = Decoder()(trainXD, e_output, m1, m2)
      y_output = Dense(9000, activation='softmax')(d_output)
      
      decoder 부분을 떼어버리면 classification문제가 된다.
      encoder 부분을 BERT로 바꾸면 decoder와 함께 번역기, 챗봇 시스템을 만들 수 있다.
...
class Encoder:
    def __init__(self,
        ...
        self.enc_layers = [
            EncoderLayer(d_model=d_model, # 6
...
class MultiHeadAttention:
    ...
    def __call__(self, q, k, v, mask):
      # x-> q, k, v
        ...
        q = self.reshapeq(q) # Reshape(-1, num_heads, d_model//num_heads)
        k = self.reshapek(k)
        v = self.reshapev(v)
...
class Attention(Layer):
    def __call__(self, inputs):
        ...
        scaled_attention_logits = matmul_qk / tf.math.sqrt(dk) # (Q*K^T)/sqrt(dk)

        scaled_attention_logits += mask * -1e9 # (Q*K^T)/sqrt(dk) + (mask * -1e9)

        attention_weights = tf.nn.softmax(scaled_attention_logits, axis=-1)

        output = tf.matmul(attention_weights, v) # ((Q*K^T)/sqrt(dk) + (mask * -1e9)) * V
